...
internal class PluginRegister
{
    ...
    /// </summary>
    public static IReadOnlyCollection<Type> ClientTypes => LazyInstance.Value._clientTypes;
    ...
    private void ActivateExtensions()
    {
        foreach (var extension in _extensions)
        {
            try
            {
                Activator.CreateInstance(extension.Type, AppControl.Instance);
            }
            catch (Exception e)
            {
    ...
    private void Initialize()
    {
        ...
        var clientTypes = AppDomain.CurrentDomain
            .GetAssemblies()
            .SelectMany(SafeGetTypes)
            .Where(t => t is {IsClass: true, IsAbstract: false})
            .Where(t => typeof(IPluginClient).IsAssignableFrom(t));
