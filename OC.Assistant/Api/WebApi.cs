...
public static class WebApi
{
    ...
    public static void RunDetached()
    {
        ...
        Task.Run(async () =>
        {
            ...
            builder.WebHost.
                UseUrls($"http://{appSettings.IpAddress}:{appSettings.WebApiPort}");
    ...
    private static IResult HandleTimeScaling(double timeScaling)
    {
        try
        {
            AppControl.Instance.UpdateTimeScaling(timeScaling);
    ...
    private static IResult HandleApplicationStart()
    {
        try
        {
            AppControl.Instance.StartPlugins();
            return Results.Accepted();
        }
        catch (Exception e)
        {
    ...
    private static IResult HandleApplicationStop()
    {
        try
        {
            AppControl.Instance.StopPlugins();
            return Results.Accepted();
        }
        catch (Exception e)
        {
    ...
    private static async Task<IResult> HandleConfig(HttpRequest request)
    {
        try
        {
            ...
            await Task.Run(() =>
            {
                AppControl.Instance.UpdateConfig(config);
            });
