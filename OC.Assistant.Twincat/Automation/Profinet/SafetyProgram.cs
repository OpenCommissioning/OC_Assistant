...
internal class SafetyProgram
{
    ...
    public SafetyProgram(IEnumerable<SafetyModule> modules, string pnName)
    {
        ...
        const string configDevDataAddr = $"{Tags.INSTANCE}.stConfig.pDevData := ADR({Tags.NAME}.Inputs.{Tags.VAR_NAME});\n";
        const string configHstDataAddr = $"{Tags.INSTANCE}.stConfig.pHstData := ADR({Tags.NAME}.Outputs.{Tags.VAR_NAME});\n";
